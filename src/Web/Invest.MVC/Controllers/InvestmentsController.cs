    ...
    public class InvestmentsController : Controller
    {
        ...
        public async Task<IActionResult> Progress()
        {
            var currentDateUtc = await _context
                .InvestmentHistories
                .Where(p => p.Investor.Name != "Cédrik" && p.Investor.Name != "Anabelle" && p.Investor.Name != "Aaricia" && p.Investor.Name != "Geneviève")
                .MaxAsync(p => p.DateUtc);
            ...
            var histories = _context
                ...
                .Include(p => p.Stock)
                .Where(p =>
                    (p.Investor.Name != "Cédrik" && p.Investor.Name != "Anabelle" && p.Investor.Name != "Aaricia" && p.Investor.Name != "Geneviève") &&
                    (
